...
out center tags;`;
  ...
  const endpoints = [...new Set([OVERPASS_URL, OVERPASS_FALLBACK_URL].filter(Boolean))];
  let data;
  let lastError;

  for (const endpoint of endpoints) {
    try {
      data = await requestJson(new URL(endpoint), {
        method: 'POST',
        body,
  ...
  if (!data) throw lastError || new Error('No map search service is available');
